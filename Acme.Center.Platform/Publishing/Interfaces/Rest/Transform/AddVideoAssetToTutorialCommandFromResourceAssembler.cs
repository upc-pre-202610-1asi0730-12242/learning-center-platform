...
/// <summary>
///     Assembler responsible for transforming an <see cref="AddVideoAssetToTutorialResource"/> into an <see cref="AddVideoAssetToTutorialCommand"/>.
/// </summary>
public static class AddVideoAssetToTutorialCommandFromResourceAssembler
{
        ...
        AddVideoAssetToTutorialResource resource, int tutorialId)
    {
        if (resource == null)
        {
            throw new ArgumentNullException(nameof(resource), "AddVideoAssetToTutorialResource cannot be null when converting to command.");
        }
