...
using System;
using Microsoft.OpenApi; // Added for InvalidOperationException

namespace Acme.Center.Platform.Publishing.Interfaces.Rest.Transform;

/// <summary>
///     Assembler responsible for transforming a <see cref="Tutorial"/> domain aggregate into a <see cref="TutorialResource"/> for REST representation.
/// </summary>
public static class TutorialResourceFromEntityAssembler
{
    ...
    /// <exception cref="InvalidOperationException">Thrown if the <see cref="Tutorial.Category"/> navigation property is null,
    /// indicating an improperly loaded aggregate or data inconsistency, as Category is expected to be present.</exception>
    public static TutorialResource ToResourceFromEntity(Tutorial entity)
    {
        if (entity == null)
        {
            throw new ArgumentNullException(nameof(entity), "Tutorial entity cannot be null when converting to resource.");
        }
        ...
        {
            throw new InvalidOperationException($"Tutorial with ID {entity.Id} has a null Category. Ensure Category is properly loaded.");
        }
